...
module.exports = (client) => {
  ...
  const bilgiler = {
    oauthSecret: "CuYAXgE96XFVl-o6FPNEqebAyefrCYa_",
    callbackURL: `https://burkut.discordbot.web.tr/callback`,
    domain: `https://burkut.discordbot.web.tr/`
  };
...
//Tovbe tovbe kufurlere bak 
 ...
 const renderTemplate = (res, req, template, data = {}) => {
  ...
  app.get("/callback", passport.authenticate("discord", { failureRedirect: "/autherror" }), async (req, res) => {
    ...
    client.channels.get("746947312231776408").send(`**${client.users.get(req.user.id).tag}** adlı kullanıcı Web Paneline Discord hesabıyla giriş yaptı!`)
...
res.redirect("/panel/"+req.params.guildID+"/genel");
  ...
  app.get("/ekle", (req, res) => {
   ...
   app.get("/seviye", (req, res) => {
    res.redirect(`https://burkut.discordbot.web.tr/kullanici/${req.params.kullaniciID}/yonet`);
  });
