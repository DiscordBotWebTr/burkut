...
client.emojiler = {
  ...
  paraGitti: "746891063876386838", // X İŞARETİ
  paraGitmedi: "746891042112012419", // TİK İŞARETİ
  ...
  evet: "746891063876386838", // TİK İŞARET
  hayır: "746891042112012419", // X İŞARETİ
  acikk: "746891042112012419",
  kapalii: "746891042112012419",
  ...
  yukleniyor: "746941321528344647", // YÜKLENİYOR EMOJİ İŞTE :D
  sinirli: "746941819367063672", // TİTREYEN SİNİRLİ :D
  mutlu: "746942198972547103", // MUTLU EMOJİ
  rahatsızetme: "746942415109226606", // RAHATSIZ ETMEYİN EMOJİSİ
  çevrimiçi: "746942567698137198", // ÇEVRİMİÇİ EMOJİSİ
  yayıncı: "746943199309856808", // YAYINCI EMOJİSİ
  çevrimdışı: "746943454889902141", // ÇEVRİM DIŞI EMOJİSİ
  boşta: "746943619935764600", // BOŞTA EMOJİSİ
  bot: "746943873804140575", // BOT EMOJİSİ
  polis: "746944522336075957", // POLİS EMOJİ
  Yvar: "746891063876386838", // YETKİLERİM KOMUDUNDAKİ TİK İŞARETİ
  Yyok: "746891042112012419", // YETKİLERİM KOMUDUNDAKİ X İŞARETİ
  yan: "746944752318021743", // > GİBİ EMOJİ İŞTE :ç
  kalpSarılmalı: "746945125351030925",
  olumlu: "746891063876386838",
  olumsuz: "746891042112012419",

  // AYARLAR KOMUDUNDAKİ AÇIK KAPALI EMOJİLERİ >>>>>>>>>>>>>>>>>
  kapalıA: "746891042112012419",
  açıkA: "746891063876386838",
...
client.ayarlar = {
  ...
  dblO: "https://discordbots.org/bot/746921862944260203/vote",
  dbl: "https://discordbots.org/bot/746921862944260203",
  dbltoken:
        "",
        ...
        if (kanalD === "var") {
          if (db.has(`üyelikk_${member.id}`)) {
            const embed = new Discord.RichEmbed()
              .setColor("RANDOM")
              .setDescription(
                `<a:gold:746914603337908254> Gold üye katıldı. \`${member.user.tag}\`, Hoşgeldin \`${rol.name}\` Adlı rolün verildi <a:gold:746914603337908254>`
              );
            ...
            member.guild.channels
              .get(db.fetch(`otoRK_${member.guild.id}`))
              .send(
                `<a:duyuru:746928437536620665> <a:indir:746928844702744577> **${member.user.tag}** adlı kullanıcıya başarıyla otomatik rol olarak ayarlanmış olan \`${rol.name}\` adlı rol verildi!`
              );
        }
      }
    }
...
client.on("guildMemberRemove", async member => {
  ...
  if (db.has(`üyelikk_${member.id}`)) {
    const embed = new Discord.RichEmbed()
      ...
      .setDescription(
        `<a:gold:746914603337908254> Gold üye kayboldu. \`${
          member.user.tag
        }\` \`${db.fetch(
          `sayac_${member.guild.id}`
        )}\` üye olmamıza son \`${db.fetch(`sayac_${member.guild.id}`) -
          member.guild.members.size}\` Kişi Kaldı <a:gold:746914603337908254>`
      );
    ...
    member.guild.channels
      .get(channel)
      .send(
        `<a:duyuru:746928437536620665> <a:cik:746929419792416810> **${
          member.user.tag
  ...
  if (db.has(`üyelikk_${member.id}`)) {
    member.guild.channels.get(hgK).send(
      db.has(`girisM_${member.guild.id}`)
        ...
        : `<a:gold:746914603337908254> <@${member.user.id}> Adlı Gold Üye Katıldı! **${client.ayarlar.webpanel}** adresinden veya (\`giriş-mesaj-ayarla\` komutu ile mesajı değiştirilebilir. <a:gold:746914603337908254>`
    );
  } else
    member.guild.channels.get(hgK).send(
      db.has(`girisM_${member.guild.id}`)
    ...
    member.guild.channels.get(hgK).send(
      db.has(`cikisM_${member.guild.id}`)
        ? cikis
            ...
            .replace("{sunucu}", `**${member.guild.name}**`)
            .replace("{kişisayısı}", `**${member.guild.members.size}**`)
        : `<a:gold:746914603337908254> **${member.user.username}** Adlı Gold Üye Ayrıldı! **${client.ayarlar.webpanel}** adresinden veya (\`çıkış-mesaj-ayarla\` komutu ile mesaj değiştirilebilir. <a:gold:746914603337908254>)`
    );
  } else
    member.guild.channels.get(hgK).send(
      db.has(`cikisM_${member.guild.id}`)
...
client.on("guildMemberAdd", async member => {
  ...
  const background = await Canvas.loadImage(
    "https://i.hizliresim.com/pRwT1C.png"
  );
...
client.on("guildMemberAdd", async member => {
  ...
  const embed = new Discord.RichEmbed()
    ...
    .setDescription(
      `<a:gold:746914603337908254> ${member.user.username} Adlı Gold üye Katıldı. <a:gold:746914603337908254>`
    );
...
const youtube = new YouTube("AIzaSyAJ7zSjI6lUIMMtFkkJN1-0MvvNyfqDSi0");
const queue = new Map();
...
let i = db.fetch(`üyelikk_${msg.author.id}`)
  ...
  .setDescription(`${client.emojis.get("746914603337908254")} Hizzaya Geçin! Burada Bir Gold Üye Belirdi! <@${msg.author.id}>`)
  .setColor("BLUE")
