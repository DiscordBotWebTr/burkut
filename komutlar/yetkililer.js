...
for(var i = 0; i < message.guild.members.size; i++) {
   if(message.guild.members.array()[i].hasPermission("MANAGE_MESSAGES") && message.guild.members.array()[i].presence.status === "dnd" && !message.guild.members.array()[i].user.bot) {
      str += `${client.emojis.get(client.emojiler.rahatsiz)} ${message.guild.members.array()[i].user.tag}\n`
    }
      ...
      str += `${client.emojis.get(client.emojiler.cevrimici)} ${message.guild.members.array()[i].user.tag}\n`
    }
      ...
      str += `${client.emojis.get(client.emojiler.bosta)} ${message.guild.members.array()[i].user.tag}\n`
    }
      ...
      str += `${client.emojis.get(client.emojiler.cevrimdisi)} ${message.guild.members.array()[i].user.tag}\n`
    }
}
