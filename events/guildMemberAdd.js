...
member.setNickname(`${tag} ${member.user.username}`)
  ...
  if (db.has(`sayac_${member.guild.id}`) === true) {
    ...
    const embed = new Discord.RichEmbed()
     .setColor('RANDOM')
     .setDescription(`<:gold:754848433969561712> Gold üye belirdi. \`${member.user.tag}\`, Ayağa kalkın \`${db.fetch(`sayac_${member.guild.id}`)}\` üye olmamıza son \`${db.fetch(`sayac_${member.guild.id}`) - member.guild.members.size}\` Kişi Kaldı <:gold:754848433969561712>`)
    ...
    member.guild.channels.get(channel).send(`<a:duyuru:752184983241752637> <a:giris:754922360888492033> **${member.user.tag}** Sunucuya katıldı! \`${db.fetch(`sayac_${member.guild.id}`)}\` üye olmamıza son \`${db.fetch(`sayac_${member.guild.id}`) - member.guild.members.size}\` üye kaldı!`)
    }};
