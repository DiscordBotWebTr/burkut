...
db.add(`komutklnm_${message.author.id}`, 1)
  ...
  if (cmd) {
    ...
    if (db.has(`karalist_${message.author.id}`) === true) {
      if (!client.users.get('130352982616178689')) {
...
(message.delete())    
  .then(message => message.delete(10000));
       ...
       if(db.has(`üyelikk_${message.member.id}`)) {
if(message.channel.id !== sunucu) return message.channel.send(`<:mavi:754847774255743037> Kral, <@${message.author.id}> Burası komut kanalı değil. <#`+ sunucu+`> kanalına giderek komutlarımı ordan kullanabilirsin. <:mavi:754847774255743037>`)
  .then
(message.delete())    
  .then(message => message.delete(10000));
    }
